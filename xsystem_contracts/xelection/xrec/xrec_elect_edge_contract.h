...
class xtop_rec_elect_edge_contract final : public xvm::system_contracts::xelect_nonconsensus_group_contract_t {
    using xbase_t = xvm::system_contracts::xelect_nonconsensus_group_contract_t;

public:
    XDECLARE_DELETED_COPY_DEFAULTED_MOVE_SEMANTICS(xtop_rec_elect_edge_contract);
    XDECLARE_DEFAULTED_OVERRIDE_DESTRUCTOR(xtop_rec_elect_edge_contract);

    explicit xtop_rec_elect_edge_contract(common::xnetwork_id_t const & network_id);

    xcontract_base * clone() override { return new xtop_rec_elect_edge_contract(network_id()); }

    void setup() override;

    void on_timer(const uint64_t current_time);

    BEGIN_CONTRACT_WITH_PARAM(xtop_rec_elect_edge_contract)
    CONTRACT_FUNCTION_PARAM(xtop_rec_elect_edge_contract, on_timer);
