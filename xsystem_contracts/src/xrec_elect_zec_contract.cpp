...
void xtop_rec_elect_zec_contract::on_timer(common::xlogic_time_t const current_time) {
    ...
    XCONTRACT_ENSURE(SELF_ADDRESS().value() == sys_contract_rec_elect_zec_addr, u8"xrec_elect_zec_contract_t instance is not triggled by sys_contract_rec_elect_zec_addr");
    XCONTRACT_ENSURE(current_time <= TIME(), u8"xrec_elect_zec_contract_t::on_timer current_time > consensus leader's time");
    XCONTRACT_ENSURE(current_time + XGET_ONCHAIN_GOVERNANCE_PARAMETER(zec_election_interval) / 2 > TIME(), u8"xrec_elect_zec_contract_t::on_timer retried too many times");
