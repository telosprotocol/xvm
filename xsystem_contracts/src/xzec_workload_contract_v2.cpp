...
std::vector<xobject_ptr_t<data::xblock_t>> xzec_workload_contract_v2::get_fullblock(common::xlogic_time_t const timestamp, const uint32_t table_id) {
    ...
    get_latest_block_time += t3 - t2;
    get_full_block_time += t4 - t3;
    total_time += t5 - t1;
    xinfo("[xzec_workload_contract_v2::get_fullblock] singletimepoint: %lu, total_time: %ld, get_latest_block_time: %ld, get_full_block_time: %ld",
          timestamp,
          t5 - t1,
          t3 - t2,
          t4 - t3);
    if ((table_id+1) % 64 == 0) {
        xinfo(
            "[xzec_workload_contract_v2::get_fullblock] totaltimepoint: %lu, total_time: %ld, get_latest_block_time: %ld, get_full_block_time: %ld",
            timestamp,
...
void xzec_workload_contract_v2::accumulate_workload_with_fullblock(common::xlogic_time_t const timestamp,
    ...
    for (auto & entity : group_workload) {
        ...
        for (auto const & wl : entity.second.m_leader_count) {
            xdbg("[xzec_workload_contract_v2::accumulate_workload_with_fullblock] workload group: %s, leader: %s, workload: %u", group.to_string().c_str(), wl.first.c_str(), wl.second);
        }
...
void xzec_workload_contract_v2::on_timer(common::xlogic_time_t const timestamp) {
    ...
    // total 64 minutes, 4 minute per round(workload_collection_interval = 24)
