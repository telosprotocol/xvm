...
void xtop_zec_elect_consensus_group_contract_new::swap_election_result(common::xlogic_time_t const current_time) {
...
void xtop_zec_elect_consensus_group_contract_new::setup() {
...
void xtop_zec_elect_consensus_group_contract_new::on_timer(common::xlogic_time_t const current_time) {
...
void xtop_zec_elect_consensus_group_contract_new::elect(common::xzone_id_t const zone_id,
                                                        common::xcluster_id_t const cluster_id,
                                                        std::uint64_t const random_seed,
                                                        common::xlogic_time_t const election_timestamp) {
    auto const & zec_standby_string_property = get_property<contract_common::properties::xstring_property_t>(
        state_accessor::properties::xtypeless_property_identifier_t{data::XPROPERTY_LAST_READ_REC_STANDBY_POOL_CONTRACT_BLOCK_HEIGHT},
        common::xaccount_address_t{sys_contract_zec_standby_pool_addr});

    uint64_t read_height = static_cast<std::uint64_t>(std::stoull(zec_standby_string_property.value()));

    auto const & result = get_property<contract_common::properties::xbytes_property_t>(
    ...
    auto const & standby_result_store = contract_common::serialization::xmsgpack_t<xstandby_result_store_t>::deserialize_from_bytes(result.value());
    ...
    auto const & election_assoc_result_store_bytes_property = get_property<contract_common::properties::xbytes_property_t>(
        state_accessor::properties::xtypeless_property_identifier_t{data::XPROPERTY_CONTRACT_GROUP_ASSOC_KEY}, common::xaccount_address_t{sys_contract_zec_group_assoc_addr});
    ...
    std::vector<std::reference_wrapper<contract_common::properties::xbytes_property_t>> result_properties{std::ref(m_consensus1_result), std::ref(m_consensus2_result)};
    ...
    for (auto index = 0; index < auditor_group_count; ++index) {
        ...
        auto election_result_store = contract_common::serialization::xmsgpack_t<xelection_result_store_t>::deserialize_from_bytes(result_properties[index].get().value());
    ...
    for (uint16_t index = 0u; (index < auditor_group_count) && (auditor_rotation_num < actual_auditor_rotation_num); ++index) {
        ...
        if (elect_auditor_validator(zone_id,
            ...
            auto const & bytes = contract_common::serialization::xmsgpack_t<xelection_result_store_t>::serialize_to_bytes(election_result_store);
            result_properties[index].get().set(bytes);
    ...
    if (!genesis_elected()) {
        ...
        m_election_executed.set(to_bytes(std::string{"1"}));
    }
}

bool xtop_zec_elect_consensus_group_contract_new::elect_auditor_validator(common::xzone_id_t const & zone_id,
                                                                          common::xcluster_id_t const & cluster_id,
...
bool xtop_zec_elect_consensus_group_contract_new::elect_auditor(common::xzone_id_t const & zid,
...
bool xtop_zec_elect_consensus_group_contract_new::elect_validator(common::xzone_id_t const & zid,
                                                                  common::xcluster_id_t const & cid,
...
bool xtop_zec_elect_consensus_group_contract_new::genesis_elected() const {
    return !m_election_executed.empty();
}
