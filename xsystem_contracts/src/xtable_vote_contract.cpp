...
void xtable_vote_contract::set_vote_info(common::xaccount_address_t const & account, vote_info_map_t const & vote_info, bool b_vote) {
    ...
    XCONTRACT_ENSURE(need_upload_vote_data(TIME()), "set_vote_info is not expire");
...
void xtable_vote_contract::calc_advance_tickets(common::xaccount_address_t const & adv_account, uint64_t votes, std::map<std::string, uint64_t> & votes_table, bool b_vote, uint64_t & node_total_votes){
    ...
    if (!b_vote) {
        ...
        XCONTRACT_ENSURE(votes <= votes_table[adv_account.to_string()], "xtable_vote_contract::calc_advance_tickets: votes_table[adv_account] not enough");
...
void xtable_vote_contract::handle_votes(common::xaccount_address_t const & account, vote_info_map_t const & vote_info, bool b_vote) {
    ...
    for (auto const & entity : vote_info) {
        ...
        xdbg("[xtable_vote_contract::handle_votes] b_vote: %d, voter: %s, node: %s, votes: %u\n",
             b_vote,
             account.c_str(),
             adv_account.c_str(),
             votes);
        ...
        update_advance_tickets(address, node_total_votes);
    }
    update_table_votes_detail(account, votes_table);
}

void xtable_vote_contract::update_advance_tickets(common::xaccount_address_t const & advance_account, uint64_t tickets) {
    xdbg("[xtable_vote_contract::update_advance_tickets] adv account: %s, tickets: %llu, pid:%d\n", advance_account.c_str(), tickets, getpid());
...
uint64_t xtable_vote_contract::get_advance_tickets(common::xaccount_address_t const & advance_account) {
    ...
    {
        XMETRICS_TIME_RECORD("sysContract_tableVote_get_property_contract_pollable_key");
        value_str = MAP_GET(XPORPERTY_CONTRACT_POLLABLE_KEY, advance_account.to_string());
        if (value_str.empty()) {
            return 0;
        }
    }
...
bool xtable_vote_contract::need_upload_vote_data(const common::xlogic_time_t onchain_timer_round) {
    ...
    xdbg("[xtable_vote_contract::need_upload_vote_data] new_time_height: %llu, old_time_height: %lld, timer_interval: %d, pid: %d\n",
         new_time_height,
         old_time_height,
         timer_interval,
         getpid());
