...
void xtop_rec_elect_edge_contract_new::elect_config_nodes(common::xlogic_time_t const current_time) {
    ...
    for (auto const & nodes : nodes_info) {
        ...
        xdbg("edge static elected in %s", nodes.node_id.c_str());
    }
...
void xtop_rec_elect_edge_contract_new::setup() {
...
#    endif
    ...
    if (elect_group(
            common::xedge_zone_id, common::xdefault_cluster_id, common::xdefault_group_id, current_time, current_time, range, standby_network_result, election_network_result)) {
        m_result.set(contract_common::serialization::xmsgpack_t<xelection_result_store_t>::serialize_to_bytes(election_result_store));
    }
}

common::xnode_type_t xtop_rec_elect_edge_contract_new::standby_type(common::xzone_id_t const & zid,
