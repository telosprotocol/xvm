...
void xtop_rec_elect_edge_contract::elect_config_nodes(common::xlogic_time_t const current_time) {
    ...
    config_register.get(std::string("edge_start_nodes"), consensus_infos);
    xinfo("[edge_start_nodes] read_all :%s", consensus_infos.c_str());
    ...
    for (auto nodes : nodes_info) {
        xinfo("[edge_start_nodes] read :%s", nodes.c_str());
