...
void xtop_table_reward_claiming_contract::recv_voter_dividend_reward(uint64_t issuance_clock_height, std::map<std::string, top::xstake::uint128_t> const & rewards) {
    ...
    XCONTRACT_ENSURE(!adv_votes.empty(), "adv_votes get empty");
...
void xtop_table_reward_claiming_contract::sub_voter_record(uint64_t cur_time, xstake::xreward_record & reward_record){
    if(reward_record.unclaimed / REWARD_PRECISION <= 0){
        return;
    }
...
void xtop_table_reward_claiming_contract::claimVoterDividend() {
    ...
    sub_voter_record(cur_time, reward_record);
