...
void xtop_contract_deploy::deploy_sys_contracts() {
    ...
    deploy(common::xaccount_address_t{sys_contract_zec_reward_addr},
           ...
           "all",
           enum_broadcast_policy_t::normal,
           std::string(sys_contract_beacon_timer_addr) + ",on_timer,C," + config::xworkload_timer_interval_onchain_goverance_parameter_t::name);
    ...
    deploy(common::xaccount_address_t{sys_contract_sharding_workload_addr},
           xnode_type_t::consensus_validator,
           "",
           enum_broadcast_policy_t::normal,
           std::string(sys_contract_beacon_timer_addr) + ",on_timer,C," + config::xtableworkload_report_schedule_interval_onchain_goverance_parameter_t::name);

    deploy(common::xaccount_address_t{sys_contract_sharding_slash_info_addr},
           xnode_type_t::consensus_validator,
           "",
           enum_broadcast_policy_t::normal,
           std::string(sys_contract_beacon_timer_addr) + ",on_collect_slash_info,C," + config::xtableslash_report_schedule_interval_onchain_goverance_parameter_t::name);

    deploy(common::xaccount_address_t{sys_contract_zec_slash_info_addr},
           xnode_type_t::zec,
           "",
           enum_broadcast_policy_t::normal,
           std::string(sys_contract_beacon_timer_addr) + ",do_unqualified_node_slash,C," + config::xpunish_interval_time_block_onchain_goverance_parameter_t::name);
...
common::xnode_type_t xtop_contract_deploy::str_to_broadcast_types(const std::string & str) {
    ...
    for (auto & s : str_to_list(str)) {
        if (s == "rec") {
            ...
            types |= common::xnode_type_t::archive;
        } else if (s == "all") {
