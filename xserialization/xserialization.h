...
struct xtop_msgpack final {
    ...
    deserialize_from_string_prop(xcontract::xcontract_base const & contract, std::string const & property_name) {
        XMETRICS_TIME_RECORD(sys_contract_addr_to_string(contract.SELF_ADDRESS()) + "_get_property_time");
        try {
            auto string_value = contract.STRING_GET(property_name);
            if (string_value.empty()) {
                return T{};
            } else {
                XMETRICS_COUNTER_INCREMENT(sys_contract_addr_to_string(contract.SELF_ADDRESS()) + "_get_property_size", string_value.size());
    ...
    serialize_to_string_prop(xcontract::xcontract_base & contract, std::string const & property_name, T const & object) {
        XMETRICS_TIME_RECORD(sys_contract_addr_to_string(contract.SELF_ADDRESS()) + "_set_property_time");
        auto bytes = codec::msgpack_encode(object);
        std::string obj_str{ std::begin(bytes), std::end(bytes) };
        XMETRICS_COUNTER_INCREMENT(sys_contract_addr_to_string(contract.SELF_ADDRESS()) + "_set_property_size", obj_str.size());
