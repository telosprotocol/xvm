...
void xrole_context_t::on_block_to_db(const xblock_ptr_t & block, bool & event_broadcasted) {
    if (m_contract_info->has_broadcasts()) {
        ...
        std::string sys_contract_block_address = data::account_address_to_block_address(m_contract_info->address);
        if (sys_contract_block_address == block->get_account()) {
            ...
            if (validator_xip.slot_id() == m_driver->address().slot_id()) {
                event_broadcasted = true;  // this node is selected to broadcast
