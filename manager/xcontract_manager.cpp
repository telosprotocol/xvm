...
void xtop_contract_manager::do_on_block(const xevent_ptr_t & e) {
    if (e->major_type == xevent_major_type_chain_timer) {
        ...
        if (store_event->blk_level != base::enum_xvblock_level_table) {  // only broadcast rec/zec table
            return;
        }
        base::xvaccount_t _vaccount(store_event->owner);
        auto zone_index = _vaccount.get_zone_index();
        if (zone_index != base::enum_chain_zone_beacon_index && zone_index != base::enum_chain_zone_zec_index) {  // only broadcast rec/zec table
            return;
        }
        ...
        for (auto & pair : m_map) { // m_map : std::unordered_map<common::xaccount_address_t, xrole_map_t *>
            ...
            for (auto & pr : *(pair.second)) {  // using xrole_map_t = std::unordered_map<xvnetwork_driver_face_t *, xrole_context_t *>;
                ...
                if (event_broadcasted) {
                    return;  // if has already broadcasted, then break;
                }
            }
        }
    }
